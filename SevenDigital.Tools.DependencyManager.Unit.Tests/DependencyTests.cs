...
namespace SevenDigital.Tools.DependencyManager.Unit.Tests
{
	...
	public class DependencyTests
	{
		...
		[Test]
        public void IsConflicted_returns_false_for_identical_version_numbers()
		{
			var dependency = new Dependency();
			dependency.LibAssembly = GetNewAssemblyNameWithVersion(1, 2, 3, 4);
			dependency.ReferencedAssembly = GetNewAssemblyNameWithVersion(1, 2, 3, 4);

			Assert.That(dependency.IsConflicted, Is.False, "should not report a conflict if the version numbers are identical");
		}
		...
		public void IsMajorConflict_returns_false_for_version_numbers_with_only_3rd_number_different()
		{
			var dependency = new Dependency();
			dependency.LibAssembly = GetNewAssemblyNameWithVersion(1, 2, 3, 4);
			dependency.ReferencedAssembly = GetNewAssemblyNameWithVersion(1, 2, 5, 4);
		...
		{
			var dependency = new Dependency();
			dependency.LibAssembly = GetNewAssemblyNameWithVersion(1, 3, 3, 8);
			dependency.ReferencedAssembly = GetNewAssemblyNameWithVersion(1, 2, 3, 8);
		...
		{
			var dependency = new Dependency();
			dependency.LibAssembly = GetNewAssemblyNameWithVersion(3, 4, 3, 8);
			dependency.ReferencedAssembly = GetNewAssemblyNameWithVersion(1, 2, 3, 8);
